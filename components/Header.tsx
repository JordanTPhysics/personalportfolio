...
export default function Header() {
  ...
  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    trackLinkClick(e.currentTarget.href, e.currentTarget.dataset.trackerId!.includes("contact-link") ? "cta-click" : "link-click");
  };
  ...
  return (
    <header className="sticky top-0 z-10 border-b border-black bg-[#F7F6F2]" id="header" onMouseEnter={handleMouseEnter}>
      <div className="border-l border-r border-black w-screen">
        <div className={`flex ${isMobile ? "flex-col" : "flex-row"} justify-between`}>
          ...
          <Link href={p === "/" ? "#contact" : "/#contact"} data-tracker-id="contact-link" onClick={handleLinkClick} className="text-h3 text-center px-6 py-2 m-2 mx-auto bg-linear-to-br from-indigo-500 to-purple-500 hover:from-purple-500 hover:to-white hover:text-black hover:scale-105 duration-200 ease-in-out text-white rounded-md">Get in Touch</Link>
